...
| !         | Логическое "НЕ"                   | Левая ассоциативность        |
...
* индексируемое выражение должно обозначать коллекцию (например, строку, массив)
* индекс — выражение, которое должно приводиться к целому числу
* результат индексации может использоваться как значение, а также может стоять в левой части присваивания (если коллекция изменяема)
...
let arr = [10, 20, 30];  // литерал массива (гипотетически)
arr[1] = 99;             // присваивание элементу
```

## Литералы объектов

Литерал объекта позволяет создавать составные значения с именованными полями. Синтаксис: фигурные скобки, внутри которых через запятую перечисляются пары `имя: значение`.

Правила:

* каждое поле имеет имя (идентификатор) и выражение-значение
* порядок полей не влияет на семантику (объект неупорядочен)
* поля могут быть любого типа, включая вложенные объекты
...
Для доступа к элементам пространств имён (например, константам, функциям) используется точечная нотация.

Правила:

* идентификаторы могут быть составными, разделёнными точками
* левая часть должна обозначать пространство имён или импортированный модуль
* правая часть — имя элемента внутри этого пространства имён
* квалифицированные идентификаторы могут использоваться в выражениях и в левой части присваивания (если элемент изменяем)
...
// В другом файле или после импорта:
import Math;
start
    show Math.PI;           // доступ к константе
    let s = Math.square(5); // вызов функции из пространства имён
end
```
...
(* ПЕРВИЧНОЕ ВЫРАЖЕНИЕ (с индексацией и объектами) *)
